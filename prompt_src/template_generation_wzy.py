...
class LMBFFTemplateGenerationTemplate(ManualTemplate):
    ...
    def wrap_one_example(self,
        ...
        ''' *** MODIFY ***
        Randomly select the label words rather than only [0]
        '''
        wrapped_examples = []
        # print('#label words: ', len(self.verbalizer.label_words[example.label]))
        for word in self.verbalizer.label_words[example.label]:
            # print(word)
            example.meta['labelword'] = word.strip()
            wrapped_example = super().wrap_one_example(example)
            wrapped_examples.append(wrapped_example)
        return wrapped_examples

class TemplateGenerator:
    r""" This is the automatic template search implementation for `LM-BFF <https://arxiv.org/pdf/2012.15723.pdf>`_. 
    It uses a generation model to generate multi-part text to fill in the template. 
    By jointly considering all samples in the dataset, it uses beam search decoding method to generate 
    a designated number of templates with the highest probability. 
    The generated template may be uniformly used for all samples in the dataset.
    ...
    def _register_buffer(self, data):
        """
        add the input into the model buffer (saved by model, but no regrads)
        """
            ...
            for item in current_output:
                ...
                for t in range(turn):
                    ...
                    with torch.no_grad():
                        ...
                        # print('aggr_output.shape', aggr_output[-1].shape) # 32 * 20 * num_of_token
                        
                aggr_output = torch.cat(aggr_output, 0) # num_training_examples * 20 * num_of_token
                
                # Gather results across all input sentences, and sort generated tokens by log likelihood
                
                aggr_output = aggr_output.mean(0) # 所有训练集样本的logits 取平均      [1 * 20 * num_of_token]
                
                log_denominator = torch.logsumexp(aggr_output[i], -1).item() # 算logits softmax 的分母
                ids = list(range(inner_model.config.vocab_size))
                ids.sort(key=lambda x: aggr_output[i][x].item(), reverse=True) # logits 对应的 token_id 排序
                
                ids = ids[:self.beam_width+3] # 排位前103 个 token

                # print('generated tokens:')
                # print([tmp_tokenizer.decode(i) for i in ids])
